...
PIPELINE_DIR = ifelse(basename(normalizePath(getwd(), winslash = "/", mustWork = TRUE)) == "tcrnet", ".", "tcrnet")
setwd(PIPELINE_DIR)

TCRNET_NPROC = as.integer(Sys.getenv("TCRNET_NPROC", unset = "8"))
VDJTOOLS_PATH = Sys.getenv("VDJTOOLS_PATH", unset = "/software/bin/vdjtools")
VDJDB_SLIM_PATH = Sys.getenv("VDJDB_SLIM_PATH", unset = "../vdjdb_release/vdjdb.slim.txt")
POOLS_PATH = Sys.getenv("POOLS_PATH", unset = "pools/")
RESULTS_DIR = Sys.getenv("RESULTS_DIR", unset = "../results/tcrnet")
FIGURES_DIR = Sys.getenv("FIGURES_DIR", unset = "../figures")
...
if (!dir.exists(POOLS_PATH)) {
  system("wget https://zenodo.org/records/19520535/files/pools.zip")
...
df.mhc.info %>%
  merge(df.cc.sel) %>%
  fwrite(file.path(RESULTS_DIR, "cluster_members.txt"), sep = "\t")
```
...
df.pwms.info.h %>%
  merge(df.mhc.info) %>%
  fwrite(file.path(RESULTS_DIR, "motif_pwms.txt"), sep = "\t")
```
...
pdf(file.path(FIGURES_DIR, "fig2a.pdf"), width = 8, height = 6)
p7
dev.off()
```
...
pdf(file.path(FIGURES_DIR, "fig2b.pdf"), width = 8, height = 3.5)
p8
dev.off()
...
pdf(file.path(FIGURES_DIR, "fig2c.pdf"), width = 8, height = 3.5)
p9
dev.off()
```
